...
class IP102(Dataset):
    def __init__(self, root, is_train=True, transform=None):
        ...
        train_txt_file = open(os.path.join(self.root, 'train.txt'))
        test_txt_file = open(os.path.join(self.root, 'test.txt'))
        ...
        train_file_list = [x for x in train_name_list]
        test_file_list = [x for x in test_name_list]

        if self.is_train:
            self.train_img = [(os.path.join(self.root, 'images', train_file)) for train_file in
                              train_file_list]
            self.train_label = [x for x in train_label_list]
        if not self.is_train:
            self.test_img = [(os.path.join(self.root, 'images', test_file)) for test_file in
                             test_file_list]
            self.test_label = [x for x in test_label_list]

    def __getitem__(self, index):
        if self.is_train:
            img = Image.open(self.train_img[index]).convert('RGB')
            target = self.train_label[index]
            if self.transform is not None:
                img = self.transform(img)
        else:
            img = Image.open(self.test_img[index]).convert('RGB')
